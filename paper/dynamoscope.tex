...
$\mu \in [+0.58,+0.97]$. This is not a framework failure: the Hopf
limit-cycle radius near onset is $r(\mu) \propto \sqrt{\mu}$, so the
cycle is mathematically present arbitrarily close to $\mu=0$ but
\emph{visually} indistinguishable from a fixed point at 64$\times$64
pixel resolution until $\sqrt{\mu}$ exceeds approximately one
diameter unit. With our render the cycle becomes spatially distinct
around $\mu \approx 0.5$, which is consistent with the observed
detection point. We frame this as a Heisenberg-style resolution
trade-off intrinsic to visual sensing: \emph{mathematical}
bifurcation at $\mu^* = 0$ and \emph{visually detectable} bifurcation
at $\mu_{\text{vis}} \approx 0.5$ are not the same event, and any
video-based detector is upper-bounded by the latter. A higher-
resolution render or a multi-scale wavelet pre-processing would lower
$\mu_{\text{vis}}$ but cannot reduce it to zero. The regime-kind
transitions reported by the classical pipelines (e.g.\ \texttt{cycle}
$\to$ \texttt{strange} for \texttt{delay\_entropy} at frame 230)
locate the visual transition correctly.
