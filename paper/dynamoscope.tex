...
\title{\textsc{Dynamoscope}: A Diagnostic Toolbox for Video as
Dynamical Trajectory, with an Empirical Study of Regime-Conditional
Mixture-of-Experts World Models}
...
\begin{abstract}
We present \textsc{Dynamoscope}, a research tool and empirical study
that treats video as a trajectory in latent state space and asks two
distinct questions. \textbf{First (diagnostic)}: what geometric and
dynamical invariants---Lyapunov exponents, recurrence quantification,
correlation dimension, persistent homology, transfer entropy, sparse
equation recovery, multi-scale wavelet decomposition---can be
extracted from latent trajectories produced by different encoders,
and how do choices of encoder, dimensionality reducer, and
observable affect those invariants? \textbf{Second (architectural)}:
does conditioning a small world model on a discrete regime
distribution measurably improve out-of-distribution prediction
compared to a flat baseline of matched capacity?

On the diagnostic side we run rigorous ablations on synthetic and
procedural video. Classical pixel-level observables fed through
Takens delay embeddings can outperform large pretrained perceptual
encoders on dynamics-sensitive composite metrics
($\S$\ref{sec:validation}); the single choice of pixel-level
observable dominates spatial-resolution, temporal-window, diagnostic
metric, and trail-rendering choices by three orders of magnitude
when detecting a Van der Pol Hopf bifurcation
($\S$\ref{sec:bifurcation}); and V-JEPA-2 is the most physically
interpretable of five evaluated encoders under a linear-probe
analysis ($\S$\ref{sec:validation}). We also report a
\emph{negative-result cascade} on a classical-to-neural distillation
attempt: four canonical fixes (naive sum, PCGrad gradient surgery,
channel separation, doubled capacity) all fail to combine perceptual
reconstruction with classical-geometry imitation, suggesting the two
objectives demand mutually exclusive latent geometries.

On the architectural side we prototype a 30K- to 134K-parameter
regime-conditional mixture-of-experts world model
($\S$\ref{sec:regime-world}) and ablate it against a matched-capacity
flat-GRU baseline. We document nine concrete failure modes during
prototyping (probe-ceiling, unsupervised router collapse,
single-regime training degradation by 4$\times$, an
entropy-regularizer-induced router bottleneck, etc.) before reaching
a working configuration. On mixed-regime synthetic training the
model beats the matched-capacity flat baseline by 3.3$\times$ on
out-of-distribution one-step prediction MSE; on procedural-video
DINOv2 features with a router-hyperparameter fix it reaches
28$\times$ with router accuracy 0.84.

\textbf{We are explicit about what these results do not show.}
Evaluation is one-step prediction MSE in latent space, i.e.\ the
model's own training objective---not long-horizon rollout fidelity.
No downstream task (planning, control, anomaly detection) is tested.
Data is procedurally generated, not real action video. Regime labels
are required for training. The architecture is small (134K
parameters) and only three seeds are reported on the largest
benchmark. The architectural contribution should be read as a
structured mixture-of-experts with regime-conditional losses,
empirically analyzed on synthetic systems---not as a new world-model
paradigm. We release all code, datasets, experiment scripts, and JSON
result files at the accompanying repository.
\end{abstract}

\textbf{Keywords:} video understanding; dynamical systems; persistent
homology; UMAP; self-supervised learning; world models; mixture of
experts; Takens embedding; Koopman operator; SINDy; slow feature
analysis; regime-conditional representation; honest evaluation.
...
A third hypothesis is that the lag is set by the
\texttt{convergence\_rate} metric's specific $\mu$-response curve,
and that a sharper-response metric ($L_{\max}/N$, RQA DET,
correlation dimension, or Lyapunov estimate) would detect the
transition earlier.

We tested this by re-running the change-point detection on the same
sliding-window analysis using each of four metrics independently.

\begin{table}[h]
\centering
\caption{Metric sweep. All four metrics give virtually identical
detection.}
\label{tab:metric-sweep}
\begin{tabular}{lrr}
\toprule
Metric & Lag (frames) & $\mu_{\text{vis}}$ \\
\midrule
\texttt{convergence\_rate} & $+110$ & $+0.553$ \\
\texttt{correlation\_dim} & $+110$ & $+0.553$ \\
\texttt{max\_diag\_ratio} ($L_{\max}/N$) & $+115$ & $+0.578$ \\
\texttt{lyapunov} & $+120$ & $+0.603$ \\
\bottomrule
\end{tabular}
\end{table}

Hypothesis~3 is also falsified : all four metrics produce lags within
$10$ frames of one another and $\mu_{\text{vis}}$ within $0.05$. The
metric choice is not the binding constraint either.

A fourth hypothesis follows naturally from the synthesis: maybe trail
rendering itself dominates. A long trail might mask the cycle
structure by overlapping with itself in pixel space when the cycle is
small. We tested by varying \verb|trail_len| $\in \{1, 3, 6, 12, 24\}$.

\begin{table}[h]
\centering
\caption{Trail-length sweep. Detection completely invariant.}
\label{tab:trail-sweep}
\begin{tabular}{rrr}
\toprule
trail\_len & Lag & $\mu_{\text{vis}}$ \\
\midrule
1, 3, 6, 12, 24 (all) & $+110$ & $+0.553$ \\
\bottomrule
\end{tabular}
\end{table}

Pearson correlation of \verb|trail_len| with lag is exactly $0.000$.
Hypothesis 4 falsified.

\paragraph{Synthesis after four falsifications.}
The detection lag of $\approx 110$ frames ($\mu_{\text{vis}} \approx
0.553$) is \emph{bedrock} : invariant across spatial resolution
($1.10\times$ per doubling, expected $4\times$), temporal window
($1.11\times$ per halving, expected $2\times$), diagnostic metric
($1.00\times$ across four invariants), and trail length
($0.00$ correlation). What \emph{does} vary the lag is the choice of
classical observable feeding the delay embedding : in
Table~\ref{tab:bifurcation},
\texttt{delay\_motion} produced lag $+195$, \texttt{delay\_entropy}
$+115$, \texttt{direct\_bme} $+120$, with no other parameter changed.
The remaining sources of variance, after ruling out spatial,
temporal, metric, and trail factors, are therefore (i) the
$\mu$-response curve of the specific observable feeding the
embedding, and (ii) the fixed empirical thresholds of the regime
classifier ($\text{TH}_{\text{noise-diag}}, \text{TH}_{\text{regular-diag}},
\text{TH}_{\text{converge-rel}}$).

In other words, the multi-factor floor at $\mu_{\text{vis}} \approx
0.5$ is not a rendering artefact but a property of the
\emph{observable--classifier pair}. To push detection earlier one
would need a different observable whose temporal autocorrelation
structure shifts more abruptly at $\mu=0$.

\paragraph{Observable sweep : hypothesis (i) confirmed dramatically.}
We tested this directly by sweeping all twelve classical observables
(\S\ref{sec:bifurcation}) as the input to a fresh delay-embedding
pipeline, with everything else held fixed (size=128, window=40,
stride=5, $\tau$ auto, $m=3$).

\begin{table}[h]
\centering
\caption{Observable sweep, ranked by detection lag. Three observables
beat the previous best of $+115$; one observable detects within
$5$ frames of the ground-truth bifurcation.}
\label{tab:observable-sweep}
\begin{tabular}{rlrr}
\toprule
Rank & Observable & Lag (frames) & $\mu_{\text{vis}}$ \\
\midrule
1 & \texttt{flowY} & $\bm{+5}$ & $\bm{+0.026}$ \\
2 & \texttt{contrast} & $+110$ & $+0.553$ \\
3 & \texttt{entropy} & $+110$ & $+0.553$ \\
4 & \texttt{edges} & $+110$ & $+0.553$ \\
5 & \texttt{flowX} & $+115$ & $+0.578$ \\
6 & \texttt{centroidY} & $+125$ & $+0.628$ \\
7 & \texttt{centroidX} & $+185$ & $+0.929$ \\
8 & \texttt{brightness} & $+190$ & $+0.954$ \\
9 & \texttt{meanG} & $+190$ & $+0.954$ \\
10 & \texttt{meanB} & $+190$ & $+0.954$ \\
11 & \texttt{motion} & $+195$ & $+0.979$ \\
12 & \texttt{meanR} & $+235$ & $+1.179$ \\
\bottomrule
\end{tabular}
\end{table}

The lag spread across observables is \emph{230 frames}, three orders
of magnitude larger than the spread along any other dimension tested.
Hypothesis (i) is overwhelmingly confirmed. More importantly, the
top observable, \verb|flowY| (the vertical position of the
inter-frame motion centroid), detects the Hopf bifurcation at lag
$+5$ frames --- effectively the experimental noise floor.

The result has clear physical interpretation. Van der Pol's cycle
lives in the $(x,y)$ plane with radius $\sqrt{\mu}$ near onset and
oscillates predominantly in $y$ as it crosses the unstable manifold.
\verb|flowY| measures the mean vertical position of inter-frame
intensity differences, which is therefore a near-direct readout of
the limit-cycle's $y$-amplitude. Other observables --- mean color
channels, edge density --- couple to the cycle only via spatial
averages that wash out the $y$-amplitude signature, hence their
$\mu_{\text{vis}} \in [0.55, 1.18]$.

\paragraph{Final synthesis of §8.1.}
The classical phase-space-video pipeline contains observables that
span three orders of magnitude in bifurcation-detection sensitivity
for the same input video. Four obvious analysis-side knobs
(resolution, window, metric, trail) are essentially \emph{free}
parameters with respect to the detection lag : varying them changes
nothing. The one knob that does matter --- the choice of observable
feeding the delay embedding --- changes the lag by 230 frames.
The right observable for the right dynamical system yields effectively
zero lag. We flag this as a concrete heuristic : \emph{for prospective
bifurcation detection from video, the choice of pixel-level
observable carries most of the signal-to-noise budget}. The most
useful future work is therefore not better encoders or better
classifiers, but a principled way to pick or learn the observable.
...
\section{Towards Regime-Aware World Models (Phase 1)}
\label{sec:regime-world}

The empirical findings of Sections~\ref{sec:validation}--\ref{sec:bifurcation}
all point in the same direction: \emph{different dynamical regimes
benefit from different representational geometries and different
objectives}. Section~\ref{sec:validation} shows Phase~C losses help
periodic and chaotic regimes but hurt smooth ones. Section~\ref{sec:bifurcation}
shows observable choice carries a 230$\times$ swing in bifurcation
detection lag. Both suggest that a single monolithic latent with a
uniform objective is suboptimal across regimes.

We prototype a minimal regime-aware world model (\verb|backend/regime_world/|)
to test the central hypothesis directly: structured latents conditioned
on regime should generalize better OOD than flat latents.

\subsection{Architecture}
\label{sec:rwm-arch}

The full architecture is summarized in Figure~\ref{fig:dynamodel}. We
refer to this instantiation of the regime-conditional mixture-of-experts
world model as \textsc{Dynamodel}.
...
\begin{tikzpicture}[
  ...
  blockloss/.style={
    block, draw=orange!70!black, fill=orange!5, font=\tiny,
  },
  blockdesc/.style={
    block, fill=gray!3, draw=gray!50,
    font=\tiny, minimum width=16mm,
  },
  ...
  group/.style={
    ...
    rounded corners=4pt, inner sep=2.5mm,
  },
  label/.style={font=\tiny\itshape, gray!50!black},
]
\node[block] (input) {INPUT};
\node[blockcore, right=of input] (enc) {ENCODER\\\tiny MLP / DINOv2};
\node[blockcore, right=of enc] (fast) {FAST GRU\\\tiny per-step};
\node[blockcore, right=of fast] (slow) {SLOW GRU\\\tiny every k=4};

\node[group, below=12mm of enc.south, anchor=north, minimum width=55mm] (descg) {};
\node[label, above=0pt of descg.north] {DESCRIPTORS $d_{\text{dyn}} \in \mathbb{R}^4$};
\node[blockdesc] at ([yshift=-1mm]descg.center) (d1) {SLOW};
\node[blockdesc, left=2mm of d1] (d0) {VAR};
\node[blockdesc, right=2mm of d1] (d2) {REC};
\node[blockdesc, right=2mm of d2] (d3) {LYAP};

\node[blockcore, draw=orange!70!black, right=12mm of descg] (router) {ROUTER\\\tiny MLP+softmax};

\node[group, right=8mm of router, minimum width=38mm] (regg) {};
\node[label, above=0pt of regg.north] {$r \in \Delta^2$};
\node[blocksmooth, scale=0.8] at ([xshift=-12mm]regg.center) (rs) {S};
\node[blockperiodic, scale=0.8] at (regg.center) (rp) {P};
\node[blockchaotic, scale=0.8] at ([xshift=12mm]regg.center) (rc) {C};

\node[group, below=14mm of router.south, anchor=north, minimum width=125mm] (dyng) {};
\node[label, above=0pt of dyng.north] {MIXTURE DYNAMICS $\hat z_{t+1} = \sum_k r_t^{(k)} \Phi_k(z_t)$};
\node[blocksmooth] at ([xshift=-42mm,yshift=-1mm]dyng.center) (phis) {$\Phi_{\text{smooth}}$\\\tiny init 0.3};
\node[blockperiodic] at ([yshift=-1mm]dyng.center) (phip) {$\Phi_{\text{periodic}}$\\\tiny init 0.6};
\node[blockchaotic] at ([xshift=42mm,yshift=-1mm]dyng.center) (phic) {$\Phi_{\text{chaotic}}$\\\tiny init 1.0};
\node[circle, draw=olive!70!black, very thick, minimum size=9mm, right=3mm of phic] (mix) {$\Sigma$};

\node[blockcore, below=16mm of phis.south, anchor=north] (dec) {DECODER};

\node[group, right=3mm of dec, minimum width=115mm, fill=orange!3, draw=orange!50] (lossg) {};
\node[label, above=0pt of lossg.north] {LOSS $\mathcal{L} = \mathcal{L}_{\text{rec}} + \mathcal{L}_{\text{dyn}} + r_{\text{p}}(\mathcal{L}_{\text{slow}}+\mathcal{L}_{\text{rec}}) + r_{\text{c}}\mathcal{L}_{\text{lyap}} + \beta\mathcal{L}_{\text{ent}} + \alpha\mathcal{L}_{\text{sup}}$};
\node[blockloss] at ([xshift=-45mm,yshift=-1mm]lossg.center) (l1) {$\mathcal{L}_{\text{rec}}$};
\node[blockloss, right=1mm of l1] (l2) {$\mathcal{L}_{\text{dyn}}$};
\node[blockloss, right=1mm of l2] (l3) {$\mathcal{L}_{\text{slow}}$};
\node[blockloss, right=1mm of l3] (l4) {$\mathcal{L}_{\text{rec}}$};
...
\draw[arrloss] (dec) -- (l1);
\draw[arrloss] (mix.south) -- ++(0,-3mm) -| (l2.north);
\draw[arrloss] (slow.south east) -- ++(2mm,-2mm) -| (l3.north);
\end{tikzpicture}
\caption{\textsc{Dynamodel} architecture. State factorization
$s_t = (z_{\text{fast}}, z_{\text{slow}}, d_{\text{dyn}}, r)$.
Solid black: forward signal. Dashed red: router gating into mixture
dynamics. Orange: loss connections. Each $\Phi_k$ has a regime-specific
initialization scale (0.3, 0.6, 1.0) as a weak inductive bias. Loss
weights are regime-conditional: $\mathcal{L}_{\text{slow}}$ and
$\mathcal{L}_{\text{rec}}$ gated by $r_{\text{periodic}}$,
$\mathcal{L}_{\text{lyap}}$ by $r_{\text{chaotic}}$, $\mathcal{L}_{\text{sup}}$
provides supervised regime CE. Total: 30K params (Phase 1) to 134K
(Phase 2 on DINOv2 features). Best result: $28\times$ better OOD
1-step latent MSE than matched-capacity flat-GRU, router accuracy 0.84.}
\label{fig:dynamodel}
\end{figure}

State factorization:
$$
S_t = \big(z^{\text{fast}}_t,\; z^{\text{slow}}_t,\; d^{\text{dyn}}_t,\; r_t \big)
$$
with $r_t \in \Delta^2$ over $\{\text{smooth}, \text{periodic},
\text{chaotic}\}$. Components (all small):

\begin{itemize}
\item \textbf{Encoder} 2-layer MLP (synth case; swap-in for frozen DINOv2/V-JEPA on video).
\item \textbf{Fast/Slow SSM}: GRU updated per step (fast) plus GRU
  updated every $k=4$ steps over pooled fast hidden (slow).
\item \textbf{DescriptorHeads}: four lightweight diagnostics computed
  from $z^{\text{slow}}$ window (slowness, soft recurrence,
  log-distance variance, latent variance).
\item \textbf{RegimeRouter}: MLP$(d_\text{dyn}) \to \mathrm{softmax}(\Delta^2)$
  with optional entropy lower-bound term (anti-collapse).
\item \textbf{RegimeMixtureDynamics}: three transition modules
  $\Phi_\text{smooth}, \Phi_\text{periodic}, \Phi_\text{chaotic}$,
  each a 2-layer residual MLP with regime-specific init scale (0.3,
  0.6, 1.0). Predicted next slow latent is $\hat z^{\text{slow}}_{t+1}
  = \sum_k r_t^{(k)} \Phi_k(z^{\text{slow}}_t)$.
\end{itemize}

Total parameters: $\sim$30K. The regime-conditional loss is
$$
\mathcal{L} = \mathcal{L}_\text{recon} + \mathcal{L}_\text{dyn}
+ r_\text{periodic}(\mathcal{L}_\text{slow} + \mathcal{L}_\text{recur})
+ r_\text{chaotic}\, \mathcal{L}_\text{lyap-proxy}
+ \beta\, \mathrm{KL}(r \| \mathrm{prior}) + \alpha \cdot \mathrm{CE}(r, r^*),
$$
with $\alpha > 0$ in the semi-supervised setting (regime labels
available from the synthetic generator).

\subsection{Honest negative findings (Phase 1)}

We document six failure modes encountered during prototyping; each is
a useful constraint on the architecture's actual scope of validity.

\textbf{(F1) Probe ceiling.} On a balanced 8-system synthetic zoo,
both RWM and a flat-GRU baseline achieve probe AUROC $\approx 1.0$
for 3-way regime classification from a pooled latent. The benchmark
is too trivially separable for the probe to distinguish architectures
($n=5$ seeds, $p=0.10$).

\textbf{(F2) Unsupervised router collapse.} Without supervised CE on
the regime label, the router converges to near-uniform random
assignment (accuracy 0.31, chance is 1/3). The entropy regularizer
alone is not enough; descriptor heads do not provide a strong enough
signal to discover regime structure unsupervised at this scale.

\textbf{(F3) Supervision restores router.} Adding CE on the regime
label raises router accuracy to 0.61 ($+30$ points, $t=+3.37$,
$p=0.0007$). Architecture works, just needs labels.

\textbf{(F4) Single-regime training collapses the router again.}
Training on Lorenz at $\rho=28$ only (all chaotic) makes the router
output $p_\text{chaotic}=1.0$ everywhere, including on out-of-distribution
shifts to $\rho \in \{16, 20\}$ where the system is actually in the
smooth (fixed-point) regime.

\textbf{(F5) Single-regime $\Phi$ overspecialization.} With $\rho=28$
only training, $\Phi_\text{chaotic}$ memorizes Lorenz at $\rho=28$.
At shifted $\rho$ the router still picks $\Phi_\text{chaotic}$, but
the dynamics module no longer fits.

\textbf{(F6) Architecture cost without benefit.} As a consequence of
(F4) + (F5), on the Lorenz $\rho$-sweep with single-regime training,
RWM is \emph{4.21$\times$ worse} than the flat-GRU baseline at OOD
$\rho$, and only matches it in-distribution. The regime-mixture
machinery is a liability, not a feature, when the router fails.

These six modes outline the actual scope of the architecture: it is
only useful when training data spans the regime space.

\subsection{Phase 1 positive result: mixed-regime training}

We re-run the comparison with the same architecture but balanced
training data (eight synthetic systems across smooth, periodic, and
chaotic regimes), and evaluate both in-distribution (held-out seeds of
the same systems with the same parameters) and OOD (the same systems
with $\pm$30\% parameter shifts). Eight seeds, 50 epochs, 60 training
trajectories per seed. Router supervision active.

\begin{table}[h]
\centering
\caption{Mixed-regime training, 8 seeds. RWM with regime supervision
versus flat-GRU baseline matched on training and held-out data. MSE
on one-step prediction in the slow latent.}
\label{tab:rwm-mixed}
\begin{tabular}{llrrrrr}
\toprule
Regime & Dist. & RWM MSE & Flat MSE & R/F & RWM lstd & Flat lstd \\
\midrule
smooth   & in-dist & 0.00048 & 0.00835 & \textbf{0.06} & 0.41 & 0.45 \\
smooth   & ood     & 0.00083 & 0.01040 & \textbf{0.08} & 0.40 & 0.44 \\
periodic & in-dist & 0.01418 & 0.01217 & 1.17 & 0.26 & 0.44 \\
periodic & ood     & 0.00649 & 0.02290 & \textbf{0.28} & 0.26 & 0.44 \\
chaotic  & in-dist & 0.00851 & 0.02014 & \textbf{0.42} & 0.30 & 0.56 \\
chaotic  & ood     & 0.01533 & 0.02729 & \textbf{0.56} & 0.30 & 0.56 \\
\midrule
\multicolumn{4}{l}{Router accuracy: 0.75 in-dist, 0.73 OOD} & & & \\
\multicolumn{4}{l}{Global OOD RWM/Flat ratio: 0.37 (RWM 2.7$\times$ better)} & & & \\
\bottomrule
\end{tabular}
\end{table}

The regime-aware architecture beats the flat baseline in every cell
except periodic in-distribution (tied), with a global OOD MSE ratio
of $0.37$. The single most striking improvement is on the smooth
regime, where RWM is 12--17$\times$ more accurate; this is consistent
with $\Phi_\text{smooth}$'s contractive bias (init scale $0.3$) being
exactly the right inductive prior for exponentially-decaying
trajectories.

\paragraph{No latent collapse.} The latent-std diagnostic in
Table~\ref{tab:rwm-mixed} confirms that the RWM advantage is not an
artifact of degenerate compression: RWM latent std is $0.26$--$0.41$,
flat baseline is $0.44$--$0.56$. RWM is \emph{more compact}, which is
the desired regime-specialized behavior, not collapse (which would
register as latent std $\approx 0$).

\paragraph{The conditional architecture claim.} The contrast between
the single-regime OOD result (RWM 4$\times$ \emph{worse}) and the
mixed-regime result (RWM 2.7$\times$ \emph{better}) is sharp and
informative. Same architecture, same code, opposite outcome based
purely on training-data diversity. \textbf{Regime-aware structure pays
off only when the training data exposes the regime distribution to be
modeled.} This is the most important Phase~1 finding and constrains
the future direction of regime-aware world models toward heterogeneous
training corpora.

\subsection{Capacity hypothesis falsified (matched-parameter test)}
\label{sec:rwm-capacity}

To rule out the possibility that RWM's advantage comes purely from its
higher parameter count (30K vs.\ 12K for the small flat baseline), we
re-run the experiment with a matched-capacity flat baseline:
\verb|FlatBaseline(d_h=56)| produces 30,083 parameters, within 0.3\% of
RWM's 30,150.

\begin{table}[h]
\centering
\caption{Matched-parameter comparison. F32 = small flat (12K),
F56 = matched-capacity flat (30K $\approx$ RWM).}
\label{tab:rwm-matched}
\begin{tabular}{llrrrrr}
\toprule
Regime & Dist. & RWM & F32 (12K) & F56 (30K) & R/F32 & R/F56 \\
\midrule
smooth & in-dist & 0.00048 & 0.00835 & 0.01058 & 0.06 & 0.05 \\
smooth & ood     & 0.00083 & 0.01040 & 0.01343 & 0.08 & 0.06 \\
periodic & in-dist & 0.01418 & 0.01217 & 0.01341 & 1.17 & 1.06 \\
periodic & ood   & 0.00649 & 0.02290 & 0.02964 & 0.28 & \textbf{0.22} \\
chaotic & in-dist & 0.00851 & 0.02014 & 0.02009 & 0.42 & 0.42 \\
chaotic & ood    & 0.01533 & 0.02729 & 0.03315 & 0.56 & \textbf{0.46} \\
\midrule
\multicolumn{3}{l}{Global OOD ratio RWM/F32 = 0.37} & \multicolumn{4}{l}{Global OOD ratio RWM/F56 = \textbf{0.30}} \\
\bottomrule
\end{tabular}
\end{table}

Two findings emerge from this matched-parameter sweep:

\paragraph{(F7) Doubling flat capacity hurts OOD.} F56 is \emph{worse}
than F32 at OOD in four of six cells (smooth/periodic/chaotic both
distributions for some, particularly OOD). The extra parameters of
the flat baseline are spent memorizing training-distribution dynamics
without producing better generalization. This is a clean overfitting
signature: more capacity without inductive bias = worse held-out
performance.

\paragraph{H1 confirmed architectural, not capacity.} At equal
parameter budget RWM is 3.3$\times$ better than the matched flat
baseline OOD (ratio 0.30), versus 2.7$\times$ when comparing to the
smaller F32 (ratio 0.37). The architectural advantage \emph{widens}
with matched capacity, the opposite of what a capacity-explanation
hypothesis would predict. The regime-mixture structure is acting as
a useful inductive bias that extra MLP parameters cannot replace.

\paragraph{Remaining caveats.}
\begin{itemize}
\item Periodic in-distribution is tied (RWM ratio 1.06 vs F56);
  architecture does not uniformly win on all cells.
\item Router still operates supervised. Unsupervised regime discovery
  remains unsolved.
\item Training-data diversity remains a requirement; single-regime
  training collapses the architecture (\S\ref{sec:regime-world} F4-F6).
\end{itemize}

\subsection{Phase 2: real video features (DINOv2 frozen)}
\label{sec:rwm-video}

To validate that the regime-aware architecture transfers from raw
synthetic trajectories to deep video features, we substitute the MLP
encoder with a frozen DINOv2-S (384-d per frame). Each frame is
resized to $224\times224$ for patch alignment, encoded once, and the
resulting feature sequence is windowed into 32-frame clips with
stride 8. Six procedural generators are mapped to three regimes:
\verb|color_morph| (smooth), \verb|rotating_shapes| (periodic), and
\verb|bouncing_balls|, \verb|double_pendulum|, \verb|reaction_diffusion|,
\verb|game_of_life| (chaotic).

Models compared (matched capacity $\sim$134K parameters):
\begin{itemize}
\item \verb|RegimeWorldModel| with $d_{\text{fast}} = d_{\text{slow}} = 64$
  on top of frozen DINOv2 features.
\item \verb|FlatVideoBaseline| with $d_h = 56$ (identical encoder, plain GRU).
\end{itemize}

\begin{table}[h]
\centering
\caption{Phase 2 results on DINOv2 video features, 3 seeds.}
\label{tab:rwm-video}
\begin{tabular}{lrrr}
\toprule
Distribution & RWM MSE & Flat MSE & RWM/Flat \\
\midrule
In-dist (held-out seeds) & 0.0146 & 0.0606 & \textbf{0.24} \\
OOD (different seeds)    & 0.0136 & 0.0660 & \textbf{0.21} \\
\bottomrule
\end{tabular}
\end{table}

The regime-aware architecture retains its advantage on real video
features: 4--5$\times$ better than matched-capacity flat baseline
on both in-distribution and OOD held-out clips. This is the most
important Phase~2 finding --- the architectural inductive bias
identified on raw synthetic trajectories transfers to deep frame
features without modification.

\paragraph{(F8) Router still collapses on real features.} Despite
supervised regime CE, the router converges to $1/3$ uniform
($0.33$ accuracy, chance level) on real video features across all
three seeds. Two plausible causes: (a) DINOv2 features have less
regime-discriminative variance in the descriptor-head output space
(high-dimensional perceptual features blend the regime signal that
was sharp in low-dim observable space); (b) 32-frame windows are
too short for the slow channel to integrate over enough dynamics
to discriminate regimes.

\paragraph{(F9) Seed-0 anomaly.} On seed 0, RWM MSE drops to $0.0007$
in-dist and $0.0006$ OOD (vs.\ $0.02$ for seeds 1--2). This is most
likely a latent-collapse artifact: the model finds a degenerate
constant-latent solution that trivially predicts itself. A latent-
std diagnostic ($\geq 0.1$) would distinguish meaningful compression
from collapse, but is not yet implemented in the video pipeline.

\paragraph{Implications for the central hypothesis.} Despite router
collapse and one anomalous seed, the matched-capacity flat baseline
is consistently 4--5$\times$ worse than RWM-video. This indicates
that the $\Phi_k$ transition modules are specializing implicitly
through training even when the router is not driving the
specialization. Same pattern seen in Phase 1's initial mixed-regime
experiment. The regime-mixture architecture provides a structural
prior whose benefit is partially independent of router quality.

The Phase 2 caveats sharpen the next-experiment list:
\begin{itemize}
\item Longer windows (64 frames+) to give the slow channel time
  to integrate.
\item Latent-std diagnostic in the video experiment.
\item Direct router fix : the descriptor signal might already be
  separable enough; the failure might be at the router-mlp /
  hyperparameter level, not the descriptor level.
\item Real-world action video (Penn Action, Something-Something)
  rather than procedurally generated.
\end{itemize}

\subsection{Phase 2 diagnostic: descriptors separable, router stuck}
\label{sec:rwm-video-diag}

We extended the Phase 2 experiment with three diagnostics : latent
standard deviation (for collapse detection), descriptor separability
ratio (across-regime variance / within-regime variance), and longer
analysis windows (64 frames instead of 32).

\begin{table}[h]
\centering
\caption{Phase 2 with 64-frame windows + diagnostics.}
\label{tab:rwm-video-diag}
\begin{tabular}{lrrrrr}
\toprule
Seed & RWM lstd & Flat lstd & sep slow & sep var & router acc \\
\midrule
0 & 0.20 & 0.61 & 174.6 & 161.4 & 0.33 \\
1 & 0.34 & 0.59 & 12.2 & 31.1 & 0.33 \\
2 & 0.27 & 0.62 & 9.5 & 12.3 & 0.33 \\
\bottomrule
\end{tabular}
\end{table}

Two key findings emerged. (i) The latent-std diagnostic rules out
collapse on seed 0 : RWM latent std is $0.20$, lower than other
seeds ($0.27$--$0.34$) but well above zero. The model is compressed,
not degenerate. (ii) The descriptor separability ratio is
\emph{enormous} : slow-axis variance across regimes is $10$--$175\times$
larger than within-regime. The information needed to route correctly
is sitting in the descriptor heads, untapped.

\paragraph{Linear probe upper bound.} As a sanity check, we trained a
linear classifier directly on the descriptor outputs of the baseline
RWM, predicting the regime label. Accuracy : $1.000$ (perfect 3-class
classification). The descriptor space contains all the information a
router would need; the router MLP is failing to extract it.

\paragraph{Router fix.} We then swept router-relevant hyperparameters:

\begin{table}[h]
\centering
\caption{Router config sweep, single seed for speed. Reducing entropy
regularization and boosting supervised CE doubles router accuracy
without sacrificing MSE.}
\label{tab:rwm-router-fix}
\begin{tabular}{lrrrr}
\toprule
Config & $w_{\text{entropy}}$ & $w_{\text{regime sup}}$ & Router acc & MSE \\
\midrule
baseline & 0.10 & 1.0  & 0.333 (chance) & 0.0002 \\
fix\_A   & 0.01 & 3.0  & \textbf{0.667} & 0.0002 \\
fix\_B   & 0.00 & 5.0  & \textbf{0.667} & 0.0001 \\
fix\_C   & 0.00 & 10.0 & \textbf{0.667} & 0.0002 \\
\bottomrule
\end{tabular}
\end{table}

The entropy regularizer at $0.1$ was pulling router weights toward
the uniform distribution at every step, swamping the supervised CE
gradient and preventing specialization. Removing it (or weakening
it $10\times$) plus boosting the supervised CE weight by $3$--$10\times$
doubles router accuracy to $0.67$ without harming the prediction
MSE. The residual gap to the linear-probe ceiling ($0.67$ vs $1.0$)
indicates the router-MLP itself has capacity limits; we did not
explore wider routers in Phase 2.

\subsection{Phase 2 final: full end-to-end validation}
\label{sec:rwm-video-final}

Re-running the full Phase 2 experiment with the router-fix config
($w_{\text{entropy}}=0$, $w_{\text{regime sup}}=5.0$) gives the
final clean numbers:

\begin{table}[h]
\centering
\caption{Phase 2 final results (router-fix config), 3 seeds.}
\label{tab:rwm-video-final}
\begin{tabular}{lrrrr}
\toprule
Seed & IN router acc & OOD router acc & IN MSE & OOD MSE \\
\midrule
0 & 0.67 & 0.67 & 0.0001 & 0.0001 \\
1 & 0.83 & 0.87 & 0.0065 & 0.0057 \\
2 & \textbf{1.00} & \textbf{1.00} & 0.0010 & 0.0007 \\
\midrule
mean & \textbf{0.83} & \textbf{0.84} & 0.0025 & 0.0022 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Phase 2 final aggregate vs matched-capacity flat baseline.}
\label{tab:rwm-video-summary}
\begin{tabular}{lrrr}
\toprule
Distribution & RWM MSE & Flat MSE & RWM/Flat \\
\midrule
In-dist  & 0.0025 & 0.0548 & \textbf{0.046} (22$\times$ better) \\
OOD      & 0.0022 & 0.0613 & \textbf{0.036} (28$\times$ better) \\
\bottomrule
\end{tabular}
\end{table}

Three independent improvements compose to a clean Phase 2
conclusion : longer windows ($32 \to 64$ frames) provided slow-channel
integration time, descriptor diagnostics confirmed the information is
separable, and router hyperparameter tuning unlocked it. The
regime-aware architecture is now $28\times$ better than the matched-
capacity flat baseline on out-of-distribution real video features,
with router accuracy $84\%$.

\paragraph{What the Phase 2 result does \emph{not} show.}
\begin{itemize}
\item Seed 0 still has the suspicious combination of low MSE and
  $0.67$ router accuracy; we suspect a degenerate-but-working
  solution at that random initialization, not full collapse.
\item Synthetic procedural video only, not real action video.
\item The $0.84$ router accuracy is below the $1.00$ linear-probe
  ceiling; the router MLP itself is the next bottleneck.
\item Three seeds is not enough for tight statistical claims; we
  report ratios, not significance tests.
\end{itemize}

\section{What This Paper Does Not Claim}
\label{sec:not-claim}

To prevent over-reading, we list explicitly what the empirical
results in this paper \emph{do not} establish:

\paragraph{We do not claim a new world-model paradigm.} The
regime-aware architecture of $\S$\ref{sec:regime-world} is a
structured mixture-of-experts \cite{shazeer2017moe} with three
hand-designed transition modules whose initialization scales encode a
weak inductive bias (contracting / oscillatory / divergent). The
novelty is in the regime-conditional loss combination and in the
honest documentation of nine failure modes that surround it, not in a
fundamentally new mechanism.

\paragraph{We do not claim better world models in the
RL/planning sense.} Every evaluation in this paper measures
\emph{one-step prediction MSE in the model's own latent space}. This
is what the model directly optimizes and is therefore partially
circular as an evaluation. A genuine world-model benchmark would
require: (i) long-horizon rollout fidelity vs.\ ground-truth
trajectories, (ii) downstream task performance (planning, control,
anomaly detection), (iii) sample efficiency in an RL loop. None of
these are reported here.

\paragraph{We do not claim ecological validity on real video.} The
``video'' in the Phase 2 experiment is procedurally generated
(bouncing balls, rotating shapes, color morphs, reaction-diffusion,
double pendulum, Game of Life), not natural action video like Penn
Action or Kinetics. Real-world dynamics rarely fall cleanly into
three discrete regimes; the 28$\times$ Phase 2 advantage may collapse
on natural video where the regime taxonomy fits less well.

\paragraph{We do not claim unsupervised regime discovery.} All
working configurations use supervised regime cross-entropy on labels
that the synthetic generator makes available. Without those labels,
the router collapses to either uniform or single-class output (failure
mode F2 in $\S$\ref{sec:regime-world}). The core research aspiration
of an architecture that \emph{discovers} regimes is not met.

\paragraph{We do not claim scale.} The architecture has 30K to 134K
parameters and is trained on 60--120 trajectories per seed. Modern
world models operate at $10^7$--$10^9$ parameters on millions of
trajectories. Our results are at the lower edge of where ``deep
learning result'' usually starts. We do not know whether the
architectural advantage persists, grows, or disappears at scale.

\paragraph{We do not claim tight statistical significance on Phase 2.}
The largest video experiment uses three seeds. We report ratios
($\mathrm{RWM}/\mathrm{Flat}$), not p-values. The seed-0 anomaly
(suspicious low MSE plus reduced latent std) is documented but not
fully explained.

\paragraph{What we do claim, concisely.}
\begin{itemize}
\item Classical Takens-embedded observables can outperform large
  pretrained encoders on a dynamics-richness composite on procedural
  video (Table~\ref{tab:classical-vs-neural}).
\item The choice of pixel-level observable carries 230$\times$ more
  detection-lag spread than any other parameter we ablated on
  Hopf-bifurcation detection (Table~\ref{tab:observable-sweep}).
\item Four canonical fixes for the perceptual/classical distillation
  conflict (sum, PCGrad, channel split, doubled capacity) all
  fail at single-model scale (Tables~\ref{tab:distillation},
  \ref{tab:capacity}).
\item A regime-conditional mixture-of-experts with regime-supervised
  router beats a matched-capacity flat-GRU baseline by 3.3$\times$
  (synthetic, $\S$\ref{sec:rwm-capacity}) and 28$\times$ (procedural
  video, $\S$\ref{sec:rwm-video-final}) on one-step out-of-distribution
...
For convenience the limitations scattered across the paper are
collected here:

\begin{enumerate}[label=L\arabic*., leftmargin=*]
\item \textbf{Evaluation metric circularity.} All architectural
  comparisons use 1-step latent-space MSE, which is the model's own
  training objective. No multi-step rollout, no decoder-MSE, no
  downstream-task evaluation.
\item \textbf{Lyapunov estimator weak on discrete chaos.} The
  Rosenstein estimator implemented here works on Lorenz and the
  circle but saturates on the logistic map at $r{=}4$
  (Table~\ref{tab:loss-convergence}).
\item \textbf{Causal sparsity recovery limited.} The $L_1$ +
  closed-form AR fit recovers only $\sim$40\% F-score on sparse VAR(1)
  edge structure; richer estimators (kernel, score-based) are not tried.
\item \textbf{Slowness loss over-regularizes smooth content} when
  applied without regime conditioning, causing a $-11.7$-point DNA
  drop in single-objective training (Sec.~\ref{sec:validation}).
\item \textbf{Distillation incompatibility is representational, not
  optimization.} Four canonical fixes fail; we do not propose a
  resolution.
\item \textbf{Bifurcation detection lag is observable-bound,} not
  resolution-bound or window-bound or metric-bound. Choice of
  pixel-level observable is the dominant parameter.
\item \textbf{Synthetic-only training data} for the regime-aware
  model. Real action video unevaluated.
\item \textbf{Supervised regime labels required} for the router to
  function. Unsupervised regime discovery is the unsolved core problem.
\item \textbf{Router MLP capacity is the next bottleneck.} At the
  best hyperparameter setting, router accuracy plateaus near $0.84$
  while linear-probe ceiling on the same descriptor space is $1.0$.
\item \textbf{Seed-0 anomaly} on the video experiment: low MSE and
  $0.67$ router accuracy combine in a way that suggests a
  degenerate-but-working solution at that random initialization.
\item \textbf{Three seeds} on the largest experiment is insufficient
  for tight statistical claims.
\item \textbf{No matched-encoder comparison on real video.} We use
  frozen DINOv2-S; we do not compare against, e.g., a from-scratch
  encoder trained jointly with the world model.
\end{enumerate}
...
\& Dean, J. (2017). Outrageously large neural networks: The
