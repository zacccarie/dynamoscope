...
\paragraph{Synthesis of three falsifications.}
The detection lag of $\approx 100$--$120$ frames ($\mu_{\text{vis}}
\approx 0.5$) is robust across spatial resolution ($64$, $128$,
$256$ px), temporal window ($15$, $30$, $60$, $120$ frames), and
diagnostic metric (four different invariants). No single dimension
explains it. The most parsimonious remaining explanation is that the
rendered video itself loses information near $\mu=0$ : a Van der Pol
limit cycle of radius $\sqrt{\mu}\approx 0.1$--$0.3$ at small $\mu$
traces a region narrower than the trail of the playhead dot, so the
\emph{visual} signal we extract observables from cannot distinguish it
from a stationary point. This is not a spatial-pixel-resolution issue
in the strict sense (our resolution sweep falsifies that), but rather
an interaction between trail rendering width, the per-frame
classical observable computation, and the metric thresholds, none of
which dominate independently but which together produce a robust
$\mu_{\text{vis}} \approx 0.5$ floor. A genuinely different signal
representation --- multi-scale wavelet pre-processing of the trail,
or replacing the trail rendering by an analytic phase-space
projection --- would be required to push $\mu_{\text{vis}}$ below this
floor. We list this as concrete future work.
