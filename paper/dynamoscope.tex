...
The most plausible alternative explanation is that the binding
constraints come from the temporal-analysis side of the pipeline:
(a) the $40$-frame sliding window smooths sub-window dynamics;
(b) the regime classifier uses fixed empirical thresholds
($\text{TH}_{\text{noise-diag}}=0.045$,
$\text{TH}_{\text{regular-diag}}=0.65$,
$\text{TH}_{\text{converge-rel}}=0.55$); and
(c) the classical observable pipeline (luminance histogram entropy on
$32$ bins, delay embedding on a one-dimensional series, RQA on
$\sim$ embedding distances) is largely invariant to pixel count once
features are computed. Lowering the detection lag would require
adjusting these temporal/feature parameters, not the spatial render.
