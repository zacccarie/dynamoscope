...
\begin{table}[h]
\centering
\caption{DNA score on three held-out regimes. Bootstrap 95\% CI
over 5 repeats. Pretrained encoders are deterministic and produce
...
\midrule
mini-RSSM, baseline (2.8\,M) & 32.3 & 34.5 & 34.8 \\
mini-RSSM, Dynamoscope (2.8\,M) & 20.6 & \textbf{43.8} & \textbf{38.9} \\
ResNet-50 frozen (25\,M) & 40.2 & 32.4 & 38.2 \\
DINOv2-S frozen (22\,M) & \textbf{40.0} & 36.3 & 35.7 \\
\bottomrule
\end{tabular}
\end{table}

A 2.8M-parameter mini-RSSM trained with Phase~C losses surpasses
both a frozen ResNet-50 (25M) and a frozen DINOv2-S (22M) on the
periodic and chaotic DNA scores (Table~\ref{tab:baselines}). On
smooth video, pretrained models dominate, consistent with the
over-regularization finding above. This suggests Phase~C losses are
most useful when target content has nontrivial dynamics that the
losses can shape rather than suppress.
