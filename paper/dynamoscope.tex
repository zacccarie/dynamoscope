...
The incompatibility is therefore \emph{representational}, not
optimization-level : the two objectives demand mutually exclusive
latent geometries, and removing the conflicting components removes
substantive learning signal. This is a strong negative result that
the released artifact (\verb|results/pcgrad_distill.json|) and the
experiment driver (\verb|experiments/pcgrad_distill.py|) make
fully reproducible.
