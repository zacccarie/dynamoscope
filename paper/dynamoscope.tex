...
semantic labeling. Four encoders (ResNet50, ViT-B/16, DINOv2-S, CLIP
ViT-B/32) and an aligned audio modality (wav2vec2) allow direct
comparison of how different pretraining objectives shape the recovered
trajectory geometry. We argue that the gaps and clusters seen in
...
\bottomrule
\end{tabular}
\caption{Encoders shipped with \textsc{Dynamoscope}.}
\label{tab:encoders}
\end{table}
...
three image encoders produce qualitatively different latent
...
ResNet50. CLIP sits between the two. This direct visualization of
encoder bias is, in our view, one of the most pedagogically useful
...
\textsc{Dynamoscope} encodes each frame independently. Genuine
video models such as VideoMAE~\cite{tong2022videomae} or
V-JEPA~\cite{bardes2024vjepa} that attend across multiple frames
should provide qualitatively better representations of motion, and
are a natural next step. The current causal module operates only on
projected principal components, which limits its statistical power
on short sequences. The CCM implementation is a single-fixed-library
variant; a full convergence test
\cite{sugihara2012ccm} requires multiple library sizes. Persistent
homology is computed only up to $H_1$ for performance.

\subsection{Future work}

In addition to integrating native video encoders, we plan to add:
neural ODE / SDE fits to the latent trajectory to obtain smooth
parametric flows; differentiable parametric
UMAP~\cite{sainburg2021parametric} so that the projection can be
optimized jointly with downstream losses; multi-video comparison
through a learned shared embedding; a streaming variant of
persistent homology (\emph{vineyards}); and a richer cross-modal
analysis with text and language-model embeddings as additional
modalities.
