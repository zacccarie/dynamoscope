...
\paragraph{Synthesis : three mechanisms contribute.}
Combining the three falsified hypotheses, we conclude that the
$\texttt{phase\_c}$ + $\texttt{distill}$ incompatibility cannot be
attributed to a single failure mode. At least three mechanisms
contribute: (a) representational --- the two objectives demand
mutually exclusive latent geometries; (b) optimization --- gradient
projection removes signal faster than it removes conflict; (c)
capacity --- channel separation halves the dimensions each loss can
shape. A genuine fix would require either a redesigned unified
objective in which both regimes appear as limits, or a substantially
larger model where capacity is no longer the binding constraint. We
flag this as an important direction for future work but do not claim
to have resolved it.
